...
class HookManager(threading.Thread):
    """This is the main class. Instantiate it, and you can hand it KeyDown and KeyUp (functions in your own modules) which execute to parse the pyxhookkeyevent class that is returned.
...
#        return pyxhookmouseevent(storewm["handle"], storewm["name"], storewm["class"], (self.mouse_position_x, self.mouse_position_y), MessageName)
        return pyxhookmouseevent(storewm["handle"], storewm["name"], storewm["class"], cursorPos, MessageName)
...
def time_now_to_int():
    ...
    timeNowStr=str(timeNow.second)+str(timeNow.microsecond)[:3]
    return int(timeNowStr)
#######################################################################
#########################END CLASS DEF#################################
#######################################################################
